...
__global__ void reduction(float *g_odata, float *g_idata)
{
	...
	for (int d = blockDim.x>>1; d > 0; d >>= 1) {
		...
		if (tid<d+blockIdx.x*blockDim.x&&tid>=blockIdx.x*blockDim.x)  temp[tid] += temp[d+tid];
	}
	// finally, first thread puts result into global memory
	if (tid==0)	g_odata[0] = temp[0];
	if (tid==blockIdx.x*blockDim.x&&blockIdx.x*blockDim.x!=0)	g_odata[0] += temp[blockIdx.x*blockDim.x];
}
...
int main( int argc, char** argv) 
{
	...
	reduction<<<num_blocks,num_threads/num_blocks,shared_mem_size>>>(d_odata,d_idata);
